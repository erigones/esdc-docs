...
Automation of creating overlay networking in Danube Cloud
=========================================================
...
Command ``esdc-overlay`` should be run in the ssh shell on the first compute node (the one that hosts the ``mgmt01`` virtual machine).
...
Or you can create completely custom overlay rule (all nodes, port 4790, ...):
    .. code-block:: bash

        esdc-overlay create customers all "-e vxlan -p vxlan/listen_ip=0.0.0.0,vxlan/listen_port=4790 -s files -p files/config=/opt/custom/networking/customers_overlay.json -p mtu=1400"
